...
class Options
{
	...
	// appearance
	public var useNotesplashes:Bool = false;
...
class HotScrollOption extends HotOption{

	var mainSprite:FlxSprite;
	var arrows:FlxSprite;
	var insideType:FlxSprite;
	var property:String;
	private var max:Int = -1;
	private var min:Int = 0;
	private var callback:Int->String->Int->Void;
	var scrolls:Array<String>;
	var insideAnim:String;
	public function new(x:Float, y:Float, name:String, scrolls:Array<String>, insideAnim:String, property:String, ?min:Int=0,?max:Int=-1,?callback:Int->String->Int->Void){
		super(x,y); 
		...
		this.insideAnim = insideAnim;
		mainSprite = new FlxSprite(0,0);
		mainSprite.frames = Paths.getSparrowAtlas('Options2','preload');
		mainSprite.animation.addByPrefix('idle', name, 24, true);
		mainSprite.animation.play('idle');
		mainSprite.antialiasing = true;
		add(mainSprite);
		...
		arrows = new FlxSprite(mainSprite.x + mainSprite.frameWidth,mainSprite.y);
		arrows.frames = Paths.getSparrowAtlas('Options2','preload');
		arrows.animation.addByPrefix('idle', name=='AccuracySystem'?'Arrows0':'Arrows2', 24, true);
		...
		insideType = new FlxSprite(0,arrows.y);
		insideType.frames = Paths.getSparrowAtlas('Options2','preload');
		for(i in 0...scrolls.length){
			insideType.animation.addByPrefix(scrolls[i], scrolls[i], 24, true);

		}
		insideType.animation.play(insideAnim);
		insideType.antialiasing = true;

		insideType.x = name=='AccuracySystem'?arrows.x + arrows.frameWidth - 155:arrows.x + arrows.frameWidth - 200;
		add(insideType);
	...
	public override function left():Bool{
		...
		Reflect.setField(OptionUtils.options,property,value);
		insideType.animation.play(scrolls[value]);
		trace('changing option');
...
class HotJudgementOption extends HotOption{
	private var names:Array<String>;
	private var judgementNames:Array<String> = [];
	...
	public function new(x:Float, y:Float,  property:String){
		super(x,y); 
		...
		mainSprite = new FlxSprite(0,0);
		mainSprite.frames = Paths.getSparrowAtlas('Options2','preload');
		mainSprite.animation.addByPrefix('idle', 'Judgements', 24, true);
		mainSprite.animation.play('idle');
		mainSprite.antialiasing = true;
		add(mainSprite);
		...
		arrows = new FlxSprite(mainSprite.x + mainSprite.frameWidth,mainSprite.y);
		arrows.frames = Paths.getSparrowAtlas('Options2','preload');
		arrows.animation.addByPrefix('idle','Arrows2', 24, true);
		arrows.animation.play('idle');
		arrows.antialiasing = true;
		...
		insideType = new FlxSprite(0,arrows.y);
		insideType.frames = Paths.getSparrowAtlas('Options2','preload');
		for(i in 0...judgementNames.length){
			insideType.animation.addByPrefix(judgementNames[i], judgementNames[i]=='Vanilla'?'Vanilla0':judgementNames[i], 24, true);


		}
		insideType.animation.play(name);
		insideType.antialiasing = true;

		insideType.x = arrows.x + arrows.frameWidth - 210;
		add(insideType);
...
class HotCheckOption extends HotOption{
	...
	public function new(x:Float, y:Float, ?name:String, property:String, ?callback:Bool->Void){
		super(x,y);
		this.property = property;
		this.callback=callback;
		mainSprite = new FlxSprite(0,0);
		...
		check = new FlxSprite(mainSprite.x + mainSprite.frameWidth - 60,mainSprite.y);
		check.frames = Paths.getSparrowAtlas('Options2','preload');
		check.animation.addByPrefix('idle', 'Check', 24, true);
		check.animation.play('idle');
		check.antialiasing = true;

		add(check);

		check.visible = Reflect.field(OptionUtils.options,property);
...
class HotControlOption extends HotOption{

	var mainSprite:FlxSprite;
	
	public function new(x:Float, y:Float, ?name:String, property:String){
		super(x,y);
...
class HotStateOption extends HotOption
{
	private var state:FlxState;
	var mainSprite:FlxSprite;

	public function new(x:Float, y:Float, name:String,state:FlxState){
		super(x,y);
		this.state=state;
		this.name=name;
