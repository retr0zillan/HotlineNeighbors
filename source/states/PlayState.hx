...
class PlayState extends MusicBeatState
{
	...
	public static var daIns:PlayState;
	...
	public static var cachedSound:Bool = false;
	...
	override public function create()
	{
		...
		if(!cachedSound){
			FlxG.sound.cache(Paths.sound('Pause', 'preload'));
			FlxG.sound.cache(Paths.sound('inPause', 'preload'));
			FlxG.sound.cache(Paths.sound('shot', 'preload'));
			
			FlxG.sound.cache(Paths.sound('Despausa', 'preload'));
			cachedSound = true;
		...
		super.create();
		if(ModChart.globalShaders.length!=0){
			ModChart.globalShaders=[];
		}

		modchart = new ModChart(this);
		effect = new HotlineEffect();
		modchart.addCamEffect(effect);
		
		modchart.removeCamEffect(effect);
				...
				if(!InitState.isAmd)
					{
						newvhs = new CoolVhsEffect2();
							...
							modchart.addCamEffect(newvhs);
							modchart.addHudEffect(newvhs);
							modchart.addNoteEffect(newvhs);
					}
		...
		boyfriend = new Boyfriend(770, 450, daCharMan, !currentOptions.noChars);
		...
		add(camFollow);
			...
			FlxG.camera.follow(camFollow, LOCKON, Main.adjustFPS(.05));
		...
		camRating.follow(camFollow,LOCKON,Main.adjustFPS(.05));
		// FlxG.camera.setScrollBounds(0, FlxG.width, 0, FlxG.height);
		FlxG.camera.zoom = defaultCamZoom;

	

		trace('stage pos is X${FlxG.camera.scroll.x} Y${FlxG.camera.scroll.y}');

		FlxG.worldBounds.set(0, 0, FlxG.width, FlxG.height);

		FlxG.fixedTimestep = false;
		healthBar = new Healthbar(0,0,boyfriend.iconName,dad.iconName,this,'health',0,2);
		...
		*/
		...
		botplayTxt.setFormat(Paths.font("lilliput-steps.ttf"), 30, FlxColor.WHITE, RIGHT, FlxTextBorderStyle.OUTLINE,FlxColor.BLACK);
		...
		*/
		...
		scoresTextts = new FlxTypedGroup<FlxText>();
		add(scoresTextts);

		

		for(i in 0...3){
			scoreTxt = new FlxText(400.5 + (i*150), FlxG.height*.9, 0, "", 20);
			
			scoreTxt.setFormat(Paths.font("lilliput-steps.ttf"), 16, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
			scoreTxt.scrollFactor.set();
			scoreTxt.textField.background = true;
			scoreTxt.textField.backgroundColor = FlxColor.BLACK;
			scoreTxt.ID = i;

			if(i==2)
				scoreTxt.x = 400.5 + (i*180);
			
			if(currentOptions.downScroll)
				scoreTxt.y = FlxG.height*.1;
	
			scoresTextts.add(scoreTxt);
			 ...
			 realScore = new FlxText(0, 40, 0, "", 40);
			
			realScore.setFormat(Paths.font("lilliput-steps.ttf"), 40, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
			realScore.scrollFactor.set();
			realScore.textField.background = true;
			realScore.textField.backgroundColor = FlxColor.BLACK;
			add(realScore);
		...
		scoresTextts.cameras = [camHUD];
		realScore.cameras = [camHUD];
	...
	var rainbow:FlxRainbowEffect;
	var scoresText:Array<FlxText>=[];
	...
	{
		var countdownStatus:Int = 1; // 3 = show entire countdown. 2 = only sounds, 1 = non-visual countdown, 0 = skip countdown
	...
	var scoresTextts:FlxTypedGroup<FlxText>;
					...
					for(str=>spr in allMap)
						{
							...
							if(spr!=dad && spr!=boyfriend && spr!=gf)
								{
									if(spr!=null && stageMap.exists(str))
	...
	function mainMap()
		{
			for(str => spr in stage.objectID)
				{
					allMap.set(str, spr);
				}
				for(str => spr in stage.foreGroundObjID)
					{
						allMap.set(str, spr);
					}
	...
	function updateScoreText(){
		...
		/*
			...
			*/
					...
					realScore.text = ScoreUtils.botPlay?'${botplayScore}pts':'${songScore}pts';
					realScore.x = 1280 - realScore.fieldWidth;

				  

					scoresTextts.members[0].text = 'MISSES: ${judgeMan.judgementCounter.get("miss")}';
					scoresTextts.members[1].text = '${accuracyName.toUpperCase()}: ${shownAccuracy}%';
					scoresTextts.members[2].text = 'RANK: ${grade}';
	...
	var rainbowColors:Array<Int> = [0xFF0000, 0xFF7F00, 0xFFFF00, 0x00FF00, 0x0000FF, 0x4B0082, 0x8B00FF];
	var currentColorIndex:Int = 0;
	var colorChangeSpeed:Float = 0.02; 

	var sweetTimer:Float = 0;
	...
	override public function update(elapsed:Float)
	{
		...
		if(!died){
			if (health <= 0)
			{
				if(!currentOptions.noFail && !inCharter ){
					...
					openSubState(new NewGameOverSubState());
		...
		if(!InitState.isAmd)
		newvhs.update(elapsed);
		/*
		sweetTimer += elapsed;

        if (sweetTimer >= colorChangeSpeed) {
            
            sweetTimer = 0;

            var currentColor:Int = rainbowColors[currentColorIndex];
            var nextColor:Int = rainbowColors[(currentColorIndex + 1) % rainbowColors.length];

          
            var lerpedColor:Int = Std.int(FlxMath.lerp(currentColor, nextColor, 0.5)); // Usamos 0.5 para un cambio más lento

        
            realScore.color = lerpedColor;

      
            currentColorIndex = (currentColorIndex + 1) % rainbowColors.length;
        }

		FlxG.watch.addQuick('cam pos', 'camX:${FlxG.camera.scroll.x} camY:${FlxG.camera.scroll.y}');
