  ...
  public function new(stage:String,currentOptions:Options){
    ...
    switch (stage){
      ...
      case 'callcenter':
        ...
        var bg:FlxSprite = new FlxSprite(0, 0).loadGraphic(Paths.image('BG','prankcall'));
        bg.antialiasing = true;
        ...
        var ceo:FlxSprite = new FlxSprite(0, 0);
        ceo.frames = Paths.getSparrowAtlas('ceoandpoles', 'prankcall');
        ceo.animation.addByPrefix('idle', 'muelto',24, true);
        ceo.animation.play('idle');
        ceo.antialiasing = true;
        //bg.scrollFactor.set(0.9, 0.9);
        //ceo.active = false;
        add(ceo);
        objectID['ceo'] = ceo;
        ...
        var ligth:FlxSprite = new FlxSprite(0, 0);
        ligth.loadGraphic(Paths.image('ForegroundLuz','prankcall'));
        ligth.antialiasing = true;
        //bg.scrollFactor.set(0.9, 0.9);
        //ceo.active = false;
        foreground.add(ligth);
        foreGroundObjID['ligth'] = ligth;

        var overlay:FlxSprite = new FlxSprite(0, 0);
        overlay.loadGraphic(Paths.image('overlayAdd','prankcall'));
        overlay.blend = ADD;
  ...
  public function beatHit(beat){
    ...
    if(doDistractions){
      ...
      switch(curStage){
        case 'callcenter':
          if(beat==392){
            var coolVid = new VideoHandler();
            coolVid.disposeVid = false;
